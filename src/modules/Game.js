    ...
    Map.addEventListener("click", (texture) => {
      ...
      if (texture.target.classList.contains("block")) {
        ...
        if (this._Player._getTool() != null) {
          ...
          this._Player._getTool() == null &&
          this._World._getItem()._getProp() != null
        ) {
          if (
            texture.target.classList.contains("Sky") ||
            texture.target.classList.contains("SnowSky")
          ) {
            if (
              this._Player._getItemCounter(this._World._getItem()._getProp()) >
              0
            ) {
              this._UpdateWorld(
                texture.target,
                this._World._getItem()._getProp(),
                false,
                -1
              );
            }
